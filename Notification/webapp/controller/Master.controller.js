...
sap.ui.define([
	...
	return BaseController.extend("Avelon.manualnotification.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("Instid", FilterOperator.Contains, sQuery)];
			} else {
				...
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							ManualNotifId: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("ManualNotifId"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				ManualNotifId: encodeURIComponent(oItem.getBindingContext().getProperty("ManualNotifId"))
			}, bReplace);
		},
