...
sap.ui.define([
	"Avelon/manualnotification/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"Avelon/manualnotification/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function (BaseController, JSONModel, formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("Avelon.manualnotification.controller.Detail", {
		...
		onDelete: function () {
			...
			var oViewModel = this.getModel("detailView"),
				...
				sObjectHeader = this._oODataModel.getProperty(sPath + "/ManualNotifId"),
		...
		/*	var oData = this.getView().getModel("NotificationForm");
			var property = oData.getProperty("/")
			
			this.getView().getModel().remove("/NotificationInfoSet", oData, {
		...
		onEdit: function () {
			...
			this.getView().getModel("EditNotificationForm").setData(oData);*/
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getModel().metadataLoaded().then(function () {
				var sObjectPath = this.getModel().createKey("NotificationInfoSet", oParameter);
		...
		_onBindingChange: function () {
			...
			var oModel = new sap.ui.model.odata.v2.ODataModel({serviceUrl: "/AV7/sap/opu/odata/avelon/MANUALNOTIFICATION_SRV/"});
        	oModel.read("/NotificationInfoSet", {
        		filters: oFilters,
        		success: function(data){
        			var pdetailid = [];
			...
			*/
			...
			var sPath = oElementBinding.getBoundContext().getPath(),
				...
				sObjectId = oObject.ManualNotifId,
				sObjectName = oObject.Instid;
